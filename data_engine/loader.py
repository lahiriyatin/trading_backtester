...
def fetch_time_series(
    ...
    try:
        ...
        raise TwelveDataError(f"API request failed: {error}") from error

    try:
        payload = response.json()
    except ValueError as error:
        raise TwelveDataError("API response was not valid JSON") from error
    ...
    if not values:
        raise TwelveDataError(
            ...
            "The symbol or interval may not be available on the current plan."
        )
    ...
    dataframe = dataframe.rename(columns={"datetime": "timestamp"})
...
def save_raw_data(
    ...
    output_path = RAW_DATA_DIR / f"{safe_symbol}_{safe_interval}_sample.parquet"
